...
namespace Sem.Tools.CmdLine.Tests
{
    ...
    public static class ClassMenuItem
    {
        ...
        public class MenuSample
        {
            ...
            /// <summary>
            /// Sample with multiple ways to specify what should be available in the menu with parameters provided by <see cref="Menu.Show(Sem.Tools.CmdLine.MenuItem[])"/>.
            /// </summary>
            /// <returns>A task to wait for.</returns>
            [TestMethod]
            ...
            /// <summary>
            /// Tests whether the menu items can be executed when the parameters are being provided when calling "Show".
            /// </summary>
            /// <returns>A task to wait for.</returns>
            [TestMethod]
            public async Task ExecutionTestWithShowParameters()
            {
                var userInput = Enumerable.Range(0, 10)
                                          .SelectMany(x => new[] { x.ToString(CultureInfo.InvariantCulture), " " })
                                          .Take(19)
                ...
                await new[]
                {
                    ...
                    MenuItem.For<TestMenuTargetWithCtorParameter>(() => instance.VoidDoIt()),
                    MenuItem.For<TestMenuTargetWithStaticMethods>(),
                }.Show(console, new { value = string.Empty, myParameter = "hello", text = "hello", target = menuTarget, menuTarget, container = instance });
            ...
            /// <summary>
            /// Tests whether the show command accepts parameters for the ctor.
            /// </summary>
            /// <returns>A task to wait for.</returns>
            [TestMethod]
...
#pragma warning restore CA1716 // Identifiers should not match keywords
        {
            ...
            public void CreatesGoodMenuItemFromInstanceType()
            {
                ...
                Assert.AreEqual("A class containing a method to create a menu from.", target.DisplayString);
            }

            /// <summary>
            /// Tests whether the generated menu item returns a value for the console.
            /// </summary>
            /// <returns>A task to wait for.</returns>
            [TestMethod]
...
#pragma warning restore CS0108 // Member hides inherited member; missing new keyword
        {
            ...
            {
                var target = new MenuItem("Hello", p => Task.CompletedTask);
